...
export const Theme = () => {
  ...
  const currentTheme = THEMES.find((t) => t.id === theme) || THEMES[0]; // Current Theme
  ...
  }, [isOpen]); // Now depends on isOpen state
  ...
  return (
    <div className="relative" ref={dropDownRef}>
      <motion.button
        ...
        className="group relative flex items-center justify-between w-48 h-12 px-4 bg-[#1e1e2e]/80 rounded-lg border border-gray-800/50 hover:border-gray-700 cursor-pointer"
      >
        <Palette className="w-4 h-4 text-gray-400 group-hover:text-gray-300" />
        <span className="text-gray-200 text-sm text-left group-hover:text-white transition-colors tracking-wider">
          {currentTheme.label}
        </span>
        <div
          ...
          >
            ...
            {THEMES.map((t, index) => (
              <motion.button
                ...
                className={`relative group w-full flex items-center hover:cursor-pointer gap-3 px-3 py-2.5 hover:bg-[#262637] transition-all duration-200 ${
                  theme === t.id
              ...
              >
                ...
                >
                  {t.icon ? t.icon : <CircleOff className="w-4 h-4" />}
                </div>
